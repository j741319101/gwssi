...
/*     */ public abstract class BaseActionCmd
    ...
    private void handleUserSetting(JSONObject jsonObject) {
        ...
        for (String nodeId : nodeIds) {
            ...
            for (Object userObj : users) {
                ...
                /*     */
                /* 230 */
                if (StringUtils.equals(bpmInentity.getType(), "user")) {
                    /* 231 */
                    IUser user1 = userService.getUserById(bpmInentity.getId());
                    /* 232 */
                    if (user1 != null) {
                        /* 233 */
                        bpmInentity.setName(user1.getFullname());
                        /*     */
                    }
                    /*     */
                }
            ...
            if (StringUtils.contains(nodeId, "&")) {
                /* 239 */
                map.put(nodeId.split("&")[1], userList);
                continue;
                /*     */
            }
            /* 241 */
    ...
    public Boolean getIgnoreAuthentication() {
        /* 270 */
        return this.ignoreAuthentication;
        /*     */
    }
    ...
    public void setDynamicBpmIdentity(String key, List<List<SysIdentity>> bpmIdentityList) {
        /* 339 */
        List<List<SysIdentity>> list = this.dynamicIdentityMap_.get(key);
        /* 340 */
        if (CollectionUtil.isEmpty(list)) {
            /* 341 */
            list = new ArrayList<>();
            ...
            list.addAll(bpmIdentityList);
            /* 343 */
            this.dynamicIdentityMap_.put(key, list);
            /*     */
        } else {
            /* 345 */
            list.clear();
    ...
    public Map<String, List<List<SysIdentity>>> getDynamicBpmIdentity() {
        /* 355 */
        return this.dynamicIdentityMap_;
        /*     */
    }
    ...
    public List<List<SysIdentity>> getDynamicBpmIdentity(String nodeId) {
        /* 359 */
        return this.dynamicIdentityMap_.get(nodeId);
        /*     */
    }
    ...
    public void setIfremindNextUser(boolean ifremindNextUser) {
        /* 412 */
        this.ifremindNextUser = ifremindNextUser;
        /*     */
    }
    ...
    public List<String> getStartAppointDestinations() {
        /* 431 */
        return this.startAppointDestinations;
        /*     */
    }
    ...
    public void setStartAppointDestinations(List<String> startAppointDestinations) {
        /* 435 */
        this.startAppointDestinations = startAppointDestinations;
        /*     */
    }
    ...
    public void setStartAppointDestination(String startAppointDestination) {
        /* 439 */
        this.startAppointDestinations = Arrays.asList(new String[]{startAppointDestination});
        /*     */
    }
    ...
    public void addStartAppointDestination(String startAppointDestination) {
        /* 443 */
        if (this.startAppointDestinations == null) {
            /* 444 */
            this.startAppointDestinations = new ArrayList<>();
            /*     */
        }
        /* 446 */
        this.startAppointDestinations.add(startAppointDestination);
        /*     */
    }
    ...
    public void clearStartAppointDestination() {
        /* 450 */
        if (!CollectionUtils.isEmpty(this.startAppointDestinations)) {
            /* 451 */
            this.startAppointDestinations.clear();
            /*     */
        }
        /*     */
    }
    ...
    public String getDynamicSubmitTaskName() {
        /* 456 */
        return this.dynamicSubmitTaskName;
        /*     */
    }
    ...
    public void setDynamicSubmitTaskName(String dynamicSubmitTaskName) {
        /* 460 */
        this.dynamicSubmitTaskName = dynamicSubmitTaskName;
        /*     */
    }
    ...
    public void setDestination(String destination) {
        ...
        if (StringUtil.isNotEmpty(destination)) {
            /* 599 */
            setDestinations(new String[]{destination});
            /*     */
        }
        /*     */
    }
    ...
    public BaseActionCmd(FlowRequestParam flowParam) {
        ...
        setDoActionName(flowParam.getActionName());
        setDefId(flowParam.getDefId());
        setInstanceId(flowParam.getInstanceId());
        setTaskId(flowParam.getTaskId());
        setBusinessKey(flowParam.getBusinessKey());
        String destination = flowParam.getDestination();
        setActionVariables(flowParam.getVariables());
        if (StringUtil.isNotEmpty(destination))
            /*     */ if (destination.contains(",")) {
            String[] destinations = destination.split(",");
            for (int i = 0; i < destinations.length; i++) {
                if (StringUtils.contains(destinations[i], "&"))
                    /*     */ destinations[i] = destinations[i].split("&")[0];
            }
            setDestinations(destinations);
        } else if (StringUtils.isNotEmpty(destination) && StringUtils.contains(destination, "&")) {
            setDestination(destination.split("&")[0]);
        } else {
            setDestination(destination);
        }
        this.approveOrgId = flowParam.getStartOrgId();
        initSpecialParam(flowParam);
        if (CollectionUtil.isNotEmpty((Map) flowParam.getNodeUsers()))
        ...
        if (StringUtil.isNotEmpty(flowParam.getFormType()))
            /* 620 */ formType = flowParam.getFormType();
    ...
    public synchronized String executeCmd() {
        ...
        BpmTaskService bpmTaskService = AppUtil.getImplInstanceArray(BpmTaskService.class).get(0);
        if (!StringUtils.isEmpty(taskId)) {
            bpmNodeDef = bpmTaskService.getBpmNodeDef(taskId);
        } else if (StringUtils.equals("start", getActionName())) {
            BpmProcessDefService bpmProcessDefService = AppUtil.getImplInstanceArray(BpmProcessDefService.class).get(0);
            bpmNodeDef = bpmProcessDefService.getStartEvent(getDefId());
        }
        ActionType actonType = ActionType.fromKey(getActionName());
        setIfremindNextUser((StringUtils.indexOf("start,agree,signAgree,oppose,signOppose,reject,reject2Start,recall,addDoAgree", getActionName()) > -1));
        ...
        BpmContext.removeThreadNextIdentitys();
        handler.execute(this);
        String remindStr = "执行操作成功";
        if (bpmNodeDef != null) {
            String reminderScript = bpmNodeDef.getNodeProperties().getReminderScript();
            Map<String, Object> params = new HashMap<>();
            params.put("submitTaskName", bpmNodeDef.getName());
            params.put("destination", getDestination());
            remindStr = bpmTaskService.getTaskReminderStr(reminderScript, params);
        }
        return remindStr;
    }

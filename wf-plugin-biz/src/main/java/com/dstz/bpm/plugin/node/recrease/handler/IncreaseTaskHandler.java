...
@Component
public class IncreaseTaskHandler implements BuiltinActionHandler<DefualtTaskActionCmd> {
   @Resource
      ...
      if (dynamicNodes != null && !dynamicNodes.isEmpty()) {
         ...
         while(var7.hasNext()) {
            ...
            if (signTaskPluginDef.isSignMultiTask()) {
               ...
               extendConf.put("users", inUsers);
               this.recreaseSignTaskExecuter.increase(execCmd);
            } else if (dynamicTaskPluginDef.getEnabled()) {
               JSONObject nodeUsers = new JSONObject();
               nodeUsers.put(dynamicNodeId, inUsers);
               extendConf.put("dynamicNodes", nodeUsers);
               this.recreaseDynamicTaskExecuter.increase(execCmd);
            }
         }
      }
   ...
   public ActionType getActionType() {
      return ActionType.INCREASEDYNAMIC;
   }
